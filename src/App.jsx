...
export default function App({ onGoToLanding }) {
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case 'programs': return <PremiumGate feature="Programas de Treino" onLogin={() => nav('login')} onUpgrade={() => nav('upgrade')}><ProgramsScreen onBack={back} onStartSession={startProgramSession} sessionLog={sessionLog} initialProgram={initialProgramForScreen} carProfile={carProfile} setCarProfile={setCarProfile} onLogin={() => nav('login')} onUpgrade={() => nav('upgrade')} /></PremiumGate>;
      ...
      case 'community': return <CommunityScreen onBack={back} onStartExercise={openExercise} onLogin={onGoToLanding} onUpgrade={() => nav('upgrade')} />;
